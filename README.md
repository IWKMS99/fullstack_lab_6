...
Постоянные данные размещаются в Supabase Free: PostgreSQL через IPv4 session pooler и приватный S3-совместимый бакет `roomflow-files`. В форме Blueprint нужно задать `SPRING_DATASOURCE_URL` (JDBC, `sslmode=require`), `SPRING_DATASOURCE_USERNAME`, `SPRING_DATASOURCE_PASSWORD`, `S3_ENDPOINT`, `S3_PUBLIC_ENDPOINT`, `S3_REGION`, `S3_ACCESS_KEY`, `S3_SECRET_KEY`. Два S3 endpoint совпадают для Supabase. JWT-ключ и случайный пароль первого администратора генерируются Render; refresh-cookie передаётся только по HTTPS. Логин первого администратора — `admin@roomflow.local`, пароль доступен в переменной `BOOTSTRAP_ADMIN_PASSWORD` в панели Environment сервиса. Для другого адреса изменить `BOOTSTRAP_ADMIN_EMAIL` перед первым запуском. После создания администратора обе переменные `BOOTSTRAP_ADMIN_*` можно убрать из окружения и Blueprint.
...
`npm run test:coverage` контролирует frontend coverage (80% строк и 70% ветвей выбранных критических модулей). `./gradlew test` запускает быстрые unit-тесты, `./gradlew integrationTest` — интеграционные; `check` включает оба набора и JaCoCo gates: 70% строк приложения, 85% строк каждого из пяти критических сервисов. После установки Chromium команда `python scripts/run_e2e.py` собирает отдельный тестовый Compose, прогоняет шесть сценариев через настоящий браузер/API/БД/S3 и очищает тестовое окружение. Данные находятся в tmpfs; подменяется только внешнее HTTP API календаря.

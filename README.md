...
`render.yaml` описывает бесплатный Render Web Service в Frankfurt с автодеплоем `checksPass`. Для автодеплоя GitHub должен быть подключён к Render; одного публичного URL репозитория недостаточно. `Dockerfile.render` объединяет Nginx и Java в одном контейнере, `tini` обрабатывает сигналы, завершение любого из двух процессов завершает контейнер. Для лимита 512 MiB заданы явные границы Java: heap 192 MiB, metaspace 160 MiB, code cache 32 MiB, Serial GC и два логических процессора. Ограничение JIT уровнем 1 снижает затраты на запуск; оставшаяся память нужна потокам, native-библиотекам и Nginx. Порт выдаёт Render, публичный origin берётся из `RENDER_EXTERNAL_URL`.

Blueprint создаёт PostgreSQL 16 на бесплатном Render Free в той же приватной сети и автоматически передаёт приложению адрес, пользователя и случайный пароль БД. Публичный доступ к БД закрыт. Скрипт запуска преобразует `DATABASE_URL` в JDBC URL; для локального запуска остаётся поддержка `SPRING_DATASOURCE_URL`. Файлы размещаются в приватном S3-совместимом бакете Supabase Free `roomflow-files`. В форме Blueprint нужно задать `S3_ENDPOINT`, `S3_PUBLIC_ENDPOINT`, `S3_REGION`, `S3_ACCESS_KEY`, `S3_SECRET_KEY`. Два S3 endpoint совпадают для Supabase. JWT-ключ и случайный пароль первого администратора генерируются Render; refresh-cookie передаётся только по HTTPS. Логин первого администратора — `admin@roomflow.local`, пароль доступен в переменной `BOOTSTRAP_ADMIN_PASSWORD` в панели Environment сервиса. Для другого адреса изменить `BOOTSTRAP_ADMIN_EMAIL` перед первым запуском. После создания администратора обе переменные `BOOTSTRAP_ADMIN_*` можно убрать из окружения и Blueprint.

Бесплатный Render засыпает после 15 минут без запросов, поэтому первый запрос может быть медленным; локальная файловая система непостоянна. Бесплатная БД Render предназначена для демонстрации: объём 1 GB, срок работы 30 дней, автоматических резервных копий нет. Для длительной работы нужно перенести PostgreSQL или изменить тариф до истечения срока. Supabase Free имеет ограничения объёма и может приостановить неактивный проект. Актуальные условия: [Render Free](https://render.com/docs/free), [Supabase Free](https://supabase.com/pricing), [Render Blueprint](https://render.com/docs/blueprint-spec).
